...
export class QuotaDetail {
  readonly quotaDetails = signal<IQuotaDetail[]>([]);
  ...
  readonly isLoading = this.quotaDetailService.quotaDetailsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      this.quotaDetails.set(this.fillComponentAttributesFromResponseBody([...this.quotaDetailService.quotaDetails()]));
    });
    effect(() => {
  ...
  trackId = (item: IQuotaDetail): string => this.quotaDetailService.getQuotaDetailIdentifier(item);

  delete(quotaDetail: IQuotaDetail): void {
    const modalRef = this.modalService.open(QuotaDetailDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.quotaDetail = quotaDetail;
  ...
  protected refineData(data: IQuotaDetail[]): IQuotaDetail[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IQuotaDetail[]): IQuotaDetail[] {
  ...
  protected queryBackend(): void {
    ...
    this.quotaDetailService.quotaDetailsParams.set(queryObject);
  }
