...
import { sampleWithRequiredData } from '../evaluation.test-samples';
...
describe('Evaluation Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '637d3465-3a28-4513-8c01-e1294c65fab0' }], {
    ...
    expect(comp.evaluations()[0]).toEqual(expect.objectContaining({ id: '637d3465-3a28-4513-8c01-e1294c65fab0' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '637d3465-3a28-4513-8c01-e1294c65fab0' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.evaluations()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '637d3465-3a28-4513-8c01-e1294c65fab0' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to evaluationService', () => {
      const entity = { id: '637d3465-3a28-4513-8c01-e1294c65fab0' };
      vi.spyOn(service, 'getEvaluationIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.evaluationsParams()).toMatchObject({ sort: ['id,desc'] });
  });
