...
import { EvaluationDeleteDialog } from '../delete/evaluation-delete-dialog';
...
export class Evaluation {
  readonly evaluations = signal<IEvaluation[]>([]);
  ...
  readonly isLoading = this.evaluationService.evaluationsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.evaluationService.evaluationsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.evaluations.set(this.fillComponentAttributesFromResponseBody([...this.evaluationService.evaluations()]));
    });
    effect(() => {
  ...
  trackId = (item: IEvaluation): string => this.evaluationService.getEvaluationIdentifier(item);
  ...
  delete(evaluation: IEvaluation): void {
    const modalRef = this.modalService.open(EvaluationDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IEvaluation[]): IEvaluation[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.evaluationService.evaluationsParams.set(queryObject);
  }
