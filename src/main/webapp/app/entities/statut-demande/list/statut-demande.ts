...
import { StatutDemandeDeleteDialog } from '../delete/statut-demande-delete-dialog';
...
@Component({
  ...
  templateUrl: './statut-demande.html',
...
export class StatutDemande {
  readonly statutDemandes = signal<IStatutDemande[]>([]);
  ...
  readonly isLoading = this.statutDemandeService.statutDemandesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      this.statutDemandes.set(this.fillComponentAttributesFromResponseBody([...this.statutDemandeService.statutDemandes()]));
    });
    effect(() => {
  ...
  trackId = (item: IStatutDemande): string => this.statutDemandeService.getStatutDemandeIdentifier(item);

  delete(statutDemande: IStatutDemande): void {
    const modalRef = this.modalService.open(StatutDemandeDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected refineData(data: IStatutDemande[]): IStatutDemande[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IStatutDemande[]): IStatutDemande[] {
  ...
  protected queryBackend(): void {
    ...
    this.statutDemandeService.statutDemandesParams.set(queryObject);
  }
