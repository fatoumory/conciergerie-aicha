...
import { CodePromoDeleteDialog } from '../delete/code-promo-delete-dialog';
import { CodePromoService } from '../service/code-promo.service';

@Component({
  selector: 'jhi-code-promo',
  templateUrl: './code-promo.html',
...
export class CodePromo {
  readonly codePromos = signal<ICodePromo[]>([]);
  ...
  readonly isLoading = this.codePromoService.codePromosResource.isLoading;
  ...
  constructor() {
    effect(() => {
      this.codePromos.set(this.fillComponentAttributesFromResponseBody([...this.codePromoService.codePromos()]));
    });
    effect(() => {
  ...
  trackId = (item: ICodePromo): string => this.codePromoService.getCodePromoIdentifier(item);

  delete(codePromo: ICodePromo): void {
    const modalRef = this.modalService.open(CodePromoDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.codePromo = codePromo;
  ...
  protected refineData(data: ICodePromo[]): ICodePromo[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: ICodePromo[]): ICodePromo[] {
  ...
  protected queryBackend(): void {
    ...
    this.codePromoService.codePromosParams.set(queryObject);
  }
