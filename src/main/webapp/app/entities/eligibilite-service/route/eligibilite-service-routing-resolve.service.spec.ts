...
import eligibiliteServiceResolve from './eligibilite-service-routing-resolve.service';

describe('EligibiliteService routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEligibiliteService returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          eligibiliteServiceResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(eligibiliteServiceResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('9fec3727-3421-4967-b213-ba36557ca194');
