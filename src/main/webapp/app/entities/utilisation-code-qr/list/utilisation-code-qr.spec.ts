...
import { sampleWithRequiredData } from '../utilisation-code-qr.test-samples';

import { UtilisationCodeQr } from './utilisation-code-qr';

vi.useFakeTimers();

describe('UtilisationCodeQr Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: UtilisationCodeQr;
  let fixture: ComponentFixture<UtilisationCodeQr>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(UtilisationCodeQr);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '9609ad2a-cecd-47c9-a80a-730574aa5903' }], {
    ...
    expect(comp.utilisationCodeQrs()[0]).toEqual(expect.objectContaining({ id: '9609ad2a-cecd-47c9-a80a-730574aa5903' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '9609ad2a-cecd-47c9-a80a-730574aa5903' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.utilisationCodeQrs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '9609ad2a-cecd-47c9-a80a-730574aa5903' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to utilisationCodeQrService', () => {
      const entity = { id: '9609ad2a-cecd-47c9-a80a-730574aa5903' };
      vi.spyOn(service, 'getUtilisationCodeQrIdentifier');
      const id = comp.trackId(entity);
      expect(service.getUtilisationCodeQrIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.utilisationCodeQrsParams()).toMatchObject({ sort: ['id,desc'] });
  });
