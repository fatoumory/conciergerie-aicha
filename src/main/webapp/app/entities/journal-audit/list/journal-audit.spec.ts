...
import { sampleWithRequiredData } from '../journal-audit.test-samples';
...
describe('JournalAudit Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: JournalAudit;
  let fixture: ComponentFixture<JournalAudit>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(JournalAudit);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'c6f55784-881d-4e0d-a58a-399582a89830' }], {
    ...
    expect(comp.journalAudits()[0]).toEqual(expect.objectContaining({ id: 'c6f55784-881d-4e0d-a58a-399582a89830' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'c6f55784-881d-4e0d-a58a-399582a89830' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.journalAudits()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'c6f55784-881d-4e0d-a58a-399582a89830' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to journalAuditService', () => {
      const entity = { id: 'c6f55784-881d-4e0d-a58a-399582a89830' };
      vi.spyOn(service, 'getJournalAuditIdentifier');
      const id = comp.trackId(entity);
      expect(service.getJournalAuditIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.journalAuditsParams()).toMatchObject({ sort: ['id,desc'] });
  });
