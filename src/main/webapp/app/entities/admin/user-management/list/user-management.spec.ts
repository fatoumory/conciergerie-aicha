...
describe('UserManagement Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ login: 'Oriande.Huet' }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.userManagements()[0]).toEqual(expect.objectContaining({ login: 'Oriande.Huet' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ login: 'Oriande.Huet' }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    successReq.flush([{ login: 'Oriande.Huet' }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackLogin', () => {
    it('should forward to userManagementService', () => {
      const entity = { login: 'Oriande.Huet' };
