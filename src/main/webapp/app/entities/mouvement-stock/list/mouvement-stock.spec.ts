...
describe('MouvementStock Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'fc5bd5ed-2dc4-439b-bdbd-8c3315d903f7' }], {
    ...
    expect(comp.mouvementStocks()[0]).toEqual(expect.objectContaining({ id: 'fc5bd5ed-2dc4-439b-bdbd-8c3315d903f7' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'fc5bd5ed-2dc4-439b-bdbd-8c3315d903f7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.mouvementStocks()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'fc5bd5ed-2dc4-439b-bdbd-8c3315d903f7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to mouvementStockService', () => {
      const entity = { id: 'fc5bd5ed-2dc4-439b-bdbd-8c3315d903f7' };
      vi.spyOn(service, 'getMouvementStockIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.mouvementStocksParams()).toMatchObject({ sort: ['id,desc'] });
  });
