...
import { MouvementStockDeleteDialog } from '../delete/mouvement-stock-delete-dialog';
...
export class MouvementStock {
  readonly mouvementStocks = signal<IMouvementStock[]>([]);
  ...
  readonly isLoading = this.mouvementStockService.mouvementStocksResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.mouvementStockService.mouvementStocksResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.mouvementStocks.set(this.fillComponentAttributesFromResponseBody([...this.mouvementStockService.mouvementStocks()]));
    });
    effect(() => {
  ...
  trackId = (item: IMouvementStock): string => this.mouvementStockService.getMouvementStockIdentifier(item);

  delete(mouvementStock: IMouvementStock): void {
    const modalRef = this.modalService.open(MouvementStockDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IMouvementStock[]): IMouvementStock[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.mouvementStockService.mouvementStocksParams.set(queryObject);
  }
