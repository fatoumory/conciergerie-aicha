...
import { QuotaServiceDeleteDialog } from '../delete/quota-service-delete-dialog';
import { IQuotaService } from '../quota-service.model';
import { QuotaServiceService } from '../service/quota-service.service';

@Component({
  selector: 'jhi-quota-service',
  templateUrl: './quota-service.html',
...
export class QuotaService {
  readonly quotaServices = signal<IQuotaService[]>([]);
  ...
  readonly isLoading = this.quotaServiceService.quotaServicesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      this.quotaServices.set(this.fillComponentAttributesFromResponseBody([...this.quotaServiceService.quotaServices()]));
    });
    effect(() => {
  ...
  trackId = (item: IQuotaService): string => this.quotaServiceService.getQuotaServiceIdentifier(item);

  delete(quotaService: IQuotaService): void {
    const modalRef = this.modalService.open(QuotaServiceDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.quotaService = quotaService;
  ...
  protected refineData(data: IQuotaService[]): IQuotaService[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IQuotaService[]): IQuotaService[] {
  ...
  protected queryBackend(): void {
    ...
    this.quotaServiceService.quotaServicesParams.set(queryObject);
  }
