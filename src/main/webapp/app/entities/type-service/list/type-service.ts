...
import { TypeServiceDeleteDialog } from '../delete/type-service-delete-dialog';
...
export class TypeService {
  readonly typeServices = signal<ITypeService[]>([]);
  ...
  readonly isLoading = this.typeServiceService.typeServicesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      this.typeServices.set(this.fillComponentAttributesFromResponseBody([...this.typeServiceService.typeServices()]));
    });
    effect(() => {
  ...
  trackId = (item: ITypeService): string => this.typeServiceService.getTypeServiceIdentifier(item);

  delete(typeService: ITypeService): void {
    const modalRef = this.modalService.open(TypeServiceDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected refineData(data: ITypeService[]): ITypeService[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: ITypeService[]): ITypeService[] {
  ...
  protected queryBackend(): void {
    ...
    this.typeServiceService.typeServicesParams.set(queryObject);
  }
