...
import sn.orange.conciergerie.service.NotificationQueryService;
...
/**
 * REST controller for managing {@link sn.orange.conciergerie.domain.Notification}.
 */
@RestController
...
public class NotificationResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new notificationDTO, or with status {@code 400 (Bad Request)} if the notification already has an ID.
    ...
    public ResponseEntity<NotificationDTO> partialUpdateNotification(
        ...
        LOG.debug("REST request to partially update Notification : {}, {}", id, notificationDTO);
        if (notificationDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, notificationDTO.getId())) {
