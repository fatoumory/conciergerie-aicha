...
/**
 * REST controller for managing {@link sn.orange.conciergerie.domain.Profil}.
 */
@RestController
...
public class ProfilResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new profilDTO, or with status {@code 400 (Bad Request)} if the profil already has an ID.
    ...
    public ResponseEntity<ProfilDTO> partialUpdateProfil(
        ...
        LOG.debug("REST request to partially update Profil : {}, {}", id, profilDTO);
        if (profilDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, profilDTO.getId())) {
     ...
     */
    @GetMapping("")
    public List<ProfilDTO> getAllProfils(@RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload) {
        LOG.debug("REST request to get all Profils");
        if (eagerload) {
            return profilService.findAllWithEagerRelationships();
        } else {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ProfilDTO> getProfil(@PathVariable("id") UUID id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteProfil(@PathVariable("id") UUID id) {
