...
import sn.orange.conciergerie.service.ClientQueryService;
...
/**
 * REST controller for managing {@link sn.orange.conciergerie.domain.Client}.
 */
@RestController
...
public class ClientResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new clientDTO, or with status {@code 400 (Bad Request)} if the client already has an ID.
    ...
    public ResponseEntity<ClientDTO> partialUpdateClient(
        ...
        LOG.debug("REST request to partially update Client : {}, {}", id, clientDTO);
        if (clientDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, clientDTO.getId())) {
