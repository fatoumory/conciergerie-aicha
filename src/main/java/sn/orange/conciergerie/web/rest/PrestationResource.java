...
/**
 * REST controller for managing {@link sn.orange.conciergerie.domain.Prestation}.
 */
@RestController
@RequestMapping("/api/prestations")
public class PrestationResource {

    private static final Logger LOG = LoggerFactory.getLogger(PrestationResource.class);
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new prestationDTO, or with status {@code 400 (Bad Request)} if the prestation already has an ID.
    ...
    public ResponseEntity<PrestationDTO> partialUpdatePrestation(
        ...
        LOG.debug("REST request to partially update Prestation : {}, {}", id, prestationDTO);
        if (prestationDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, prestationDTO.getId())) {
    ...
    public List<PrestationDTO> getAllPrestations(
        ...
        if (eagerload) {
            return prestationService.findAllWithEagerRelationships();
        } else {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<PrestationDTO> getPrestation(@PathVariable("id") UUID id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deletePrestation(@PathVariable("id") UUID id) {
